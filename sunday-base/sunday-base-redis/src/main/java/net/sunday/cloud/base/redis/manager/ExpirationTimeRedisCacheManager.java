package net.sunday.cloud.base.redis.manager;
...
/**
 * 自定义 过期时间的 RedisCacheManager
 * 通过 {@link Cacheable#cacheNames()} 来实现自定义过期时间
 * 格式为 "key#ttl" 时，# 后面的 ttl 为过期时间。单位为最后一个字母（支持的单位有：d 天，h 小时，m 分钟，s 秒），默认单位为 s 秒
 */

public class ExpirationTimeRedisCacheManager extends RedisCacheManager {

    private static final String SPLIT = "#";


    public ExpirationTimeRedisCacheManager(RedisCacheWriter cacheWriter, RedisCacheConfiguration defaultCacheConfiguration) {
    ...
    protected @NonNull RedisCache createRedisCache(@NonNull String name, RedisCacheConfiguration cacheConfiguration) {
        ...
        return super.createRedisCache(names[0] + names[1], cacheConfiguration);
    }
